...
class PreviewView extends TemplateView
{
    ...
    protected function renderGrid(ProviderInterface $provider, array $row, Form $form)
    {
        ...
        if (!isset($renderedGrids[$row['uid']]) || $renderedGrids[$row['uid']]['count'] < 2) {
            $content = '';
            $renderedGrids[$row['uid']]['content'] = 'temp';
            ++$renderedGrids[$row['uid']]['count'];
            ...
            $renderedGrids[$row['uid']]['content'] = $content;
        }
        return $renderedGrids[$row['uid']]['content'];
    }
    ...
    protected function getInitializedPageLayoutView(ProviderInterface $provider, array $row)
    {
        ...
        $layoutConfiguration = $provider->getGrid($row)->buildExtendedBackendLayoutArray();
        ...
        $dblist->tt_contentConfig['activeCols'] = implode(',', $layoutConfiguration['__colPosList']);
        $dblist->CType_labels = [];
    ...
    protected function getPageLayoutView(ProviderInterface $provider, array $record)
    {
        ...
        $view = GeneralUtility::makeInstance(ObjectManager::class)->get(PageLayoutView::class);
